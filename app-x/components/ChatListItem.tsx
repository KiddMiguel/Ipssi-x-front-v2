...
export default function ChatListItem({ 
  id, avatar, name, lastMessage, time, isOnline, unread = 0 
}: ChatListItemProps) {
        ...
        <View className="flex-row justify-between items-center mt-1">
          <View className="flex-row items-center flex-1 mr-4">
            {lastMessage.startsWith('Vous :') && (
              <Feather name="check-circle" size={14} color="#60a5fa" className="mr-1" />
            )}
          ...
          {unread > 0 && (
            <View className="bg-blue-500 rounded-full min-w-[32px] h-6 px-2.5 items-center justify-center">
              <Text className="text-white text-xs font-bold" style={{ minWidth: 20, textAlign: 'center' }}>
                {unread}
              </Text>
            </View> 
          )}
